...
const UserCurrencyOption = ({value, text}) => {
    const {currency, toggleCurrency} = useCurrency()

    const isSelected = value == currency
    const isSelectedStyle = isSelected ? ' profile-select__option--selected' : ''

    const onClick = () => toggleCurrency(value)

    return (
        <>
            <button className={"profile-select__option" + isSelectedStyle} {...{onClick}}>
                <p><span>{value}</span> <Text content={text} /></p>
...
export default UserCurrencyOption;